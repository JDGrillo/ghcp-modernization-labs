...
- A declared disposable, single-user sandbox may use local Terraform state with the learner's interactive Microsoft Entra identity. Keep state and plan files only in the protected local workspace, exclude them from source control, and never use this mode for a shared or persistent environment.
- Store shared-environment state in a separately bootstrapped, protected Azure Storage backend with versioning, soft delete, private access, locking, and workload identity federation. Never commit or distribute state, plan files, credentials, or generated secrets.
...
- Use user-assigned managed identities for workloads, Microsoft Entra authentication for users and Azure SQL, interactive Entra authentication for a local sandbox operator, federated workload identity for CI/CD, and least-privilege data-plane RBAC. Do not use SQL passwords, client secrets, access keys, or broad Owner assignments.
