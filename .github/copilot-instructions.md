...
- Provision Azure with reviewed Terraform only; never use `azd`. An explicitly designated disposable, single-user sandbox may use local state and the learner's interactive Microsoft Entra identity when state and plan files remain uncommitted and the subscription is not shared. Shared or persistent environments require protected remote state and federated CI/CD identity. Require private data-plane connectivity, a WAF-controlled edge, managed workload identities, least privilege, diagnostics, policy checks, and approval bound to a saved-plan digest.
