...
4. Select the approved state profile. A declared disposable, single-user sandbox may use local state in a protected, source-ignored workspace and the learner's interactive Microsoft Entra identity. Shared or persistent environments must bootstrap remote state separately with versioning, soft delete, encryption, private access, locking, and narrowly scoped workload identity federation. Never put state, plans, backend credentials, or tokens in source or evidence.
...
9. Run formatting, `terraform validate`, lint, security/policy checks, tests, and a speculative plan. In the sandbox profile, generate the saved plan in the protected local workspace after confirming the operator identity, subscription, tenant, and source-control exclusions. Otherwise generate it only in protected CI or a secured private operator environment. Review replacements, deletes, role grants, network exposure, policy exceptions, costs, sensitive outputs, Azure SQL retention, and protections against accidental database destruction.
...
12. Apply the exact reviewed saved plan without `-auto-approve`. The human learner may authorize their own disposable sandbox plan, but the agent may not approve it. Do not apply an unreviewed speculative plan. Capture exit status and state serial without storing sensitive plan or state content in evidence.
