...
Build an accessible TypeScript frontend that expresses user tasks and consumes the approved framework-neutral API contract. Keep authoritative business behavior on the selected server.
...
- Repeat all security, integrity, state-transition, and authoritative validation in the selected backend.
...
- Treat route guards and hidden controls as usability only; enforce authorization in the selected backend.
...
- End-to-end tests through React, the selected backend, and Azure SQL for approved oracle cases.
